...
const createLspDB = (callback, lspName) => {
    const query = `
        INSERT INTO \`loaisanpham\` (\`Ten\`, \`NgayThem\`, \`update_at\`,\`showw\`) 
        VALUES (?, NOW(), NOW(),1)
    `;

    connection.query(query, [lspName], (err, results) => {
        if (err) {
            console.error('Error inserting into loaisanpham:', err);
            return callback(null, err); // Return error to the callback for handling
        }
